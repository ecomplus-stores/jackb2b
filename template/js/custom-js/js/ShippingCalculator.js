import {
  ...
  export default {
    ...
    computed: {
      ...
      isInfoProduct () {
        return this.shippedItems.every(item => {
            if (item.categories && item.categories.length) {
                return item.categories.every(({ _id }) => _id === '6571e4032cd6b65959d2087b')
            }
            return false
        })
      },
        ...
        if (shippingResult.length) {
          shippingResult.forEach(appResult => {
            ...
            response.shipping_services.forEach(service => {
              if (this.isInfoProduct) {
                if (service.label === 'Envio Digital') {
                  this.shippingServices.push({
                    app_id: appResult.app_id,
                    ...service
                  })
                }
              } else if (service.label !== 'Envio Digital') {
                this.shippingServices.push({
                    app_id: appResult.app_id,
                    ...service
                })
